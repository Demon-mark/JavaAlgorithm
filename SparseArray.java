...
class SparseArrayTest {
    public static void main(String[] args) {
        //创建一个大部分元素为0的12 * 12的二维数组
        int[][] sourceArr = new int[12][12];
        sourceArr[1][1] = 1;
        sourceArr[3][4] = 2;
        ...
        //创建一个稀疏数组，数组应当有sum + 1行3列
        int[][] sparseArr = new int[sum+1][3];

        //稀疏数组第一行存储原数组的行数、列数以及有效值个数(sum)
        ...
        int[][] newSparseArr = SparseArray.getSparseArray(sourceArr);
        System.out.println("对应的稀疏数组为：");
        for(int i = 0; i < newSparseArr.length; i++) {
            for(int j = 0; j < newSparseArr[0].length; j++) {
                System.out.print(newSparseArr[i][j] + "\t");
            }
            System.out.println();
        }

        int[][] newDestArr = SparseArray.getSourceArray(sparseArr);
        System.out.println("还原后的数组为：");
        for(int i = 0; i < newDestArr.length; i++) {
            for(int j = 0; j < newDestArr[0].length; j++) {
                System.out.print(newDestArr[i][j] + "\t");
            }
            System.out.println();
        }
